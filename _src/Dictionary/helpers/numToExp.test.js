import numberStringToExponential, {addBigInts} from './numToExp';


export default function test(cb, expect, T,L,D) {

  var sizeOfRandomTests = 1e3;
  ...
  .forEach(x => {
    ///L(`${x[0]} + ${x[1]} = ${addBigInts(x[0], x[1])}, ${x[2]}`);
    addBigInts(x[0], x[1]).should.equal(x[2]);
  });


  T('addBigInts(): tests with random small numbers');
  var rand = () =>
  ...
  for (var i = 0; i < sizeOfRandomTests; i++) {
    var a = rand();
    var b = rand();
    ///L(`${a} + ${b} = ${addBigInts(a, b)}, ${Number(a) + Number(b)}`);
    addBigInts(a, b).should.equal( (Number(a) + Number(b)).toString() );
  }


  T('numberStringToExponential()');
  [
    ['5.0',     '5e+0'],
    ['10.5',    '1.05e+1'],
    ['0.105E2', '1.05e+1'],
    ...
    [ '10000e-10002', '1e-9998' ],  // neg exp, pos add = neg(pos exp, subtr).
    ['0.0001e-9998' , '1e-10002'],  // neg exp, neg add = neg(pos exp, pos add).
  ...
  .forEach(x => {
    ///L(`${x[0]} = ${numberStringToExponential(x[0])}, ${x[1]}`);
    numberStringToExponential(x[0]).should.equal(x[1]);
  });


  T('numberStringToExponential(): tests with random number give same result ' +
    'as with Number.toExponential()');
  var rand = () =>
    ((Math.random() - 0.5) * 1e2).toString().substring(0, 12) + // Short random.
    'e' + ~~((Math.random() - 0.5) * 100);  // An exponent that JS can handle.
  for (var i = 0; i < sizeOfRandomTests; i++) {
    var a = rand();
    var b = numberStringToExponential(a);
    ///L(`${a} = ${b}, ${Number(a).toExponential()}`);
    b.should.equal( Number(a).toExponential() );
  }
